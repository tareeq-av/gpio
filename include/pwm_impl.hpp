#ifndef GPIO_INCLUDE_PWM_IMPL_HPP_
#define GPIO_INCLUDE_PWM_IMPL_HPP_
...
class PwmImpl : public Pwm, public Gpio<TChip, TLine, TLineRequest> {
 ...
 public:
  explicit PwmImpl(const int& pin_number):
  Gpio<TChip, TLine, TLineRequest>(pin_number, direction_) {
    ...
    virtual inline void CalculateDuration() {
      this->on_duration_ = std::chrono::duration<double>(
        this->duty_cycle_ * this->slice_time_);

      this->off_duration_ = std::chrono::duration<double>(
        (this->max_cycle_ - this->duty_cycle_) * this->slice_time_);
    }
    ...
    virtual void SetSpeed(double speed) {
      ...
      if (this->duty_cycle_ < 0.0) { this->duty_cycle_ = 0.0; }
      if (this->duty_cycle_ > 100.0 { this->duty_cycle_ = 100.0; }
      this->CalculateDuration();
    }
...
#endif // GPIO_INCLUDE_PWM_IMPL_HPP_
